...
CREATE TABLE mlops.CV_SPLIT_ROW (
    ...
    row_key_text NVARCHAR(900),
    row_key_digest VARBINARY(32) NOT NULL,
    test_fold_no INT NOT NULL,
    PRIMARY KEY (split_set_id, row_ordinal),
    FOREIGN KEY (split_set_id) REFERENCES mlops.CV_SPLIT_SET(split_set_id),
    FOREIGN KEY (split_set_id, test_fold_no) REFERENCES mlops.CV_FOLD(split_set_id, fold_no)
);
...
CREATE TABLE pricing.RATE_PACKAGE (
    ...
    created_ts DATETIME2(3) DEFAULT SYSDATETIME(),
    created_by NVARCHAR(128) NOT NULL,
    FOREIGN KEY (parent_rate_package_id) REFERENCES pricing.RATE_PACKAGE(rate_package_id),
    FOREIGN KEY (model_id) REFERENCES pricing.MODEL(model_id),
    FOREIGN KEY (model_run_id) REFERENCES mlops.MODEL_RUN(model_run_id)
);
...
CREATE TABLE pricing.RATE_CELL_LEVEL (
    ...
    FOREIGN KEY (feature_level_id) REFERENCES pricing.FEATURE_LEVEL(feature_level_id)
);
