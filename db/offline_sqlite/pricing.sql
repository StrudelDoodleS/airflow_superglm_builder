...
CREATE TABLE IF NOT EXISTS pricing.MODEL_RUN (
    ...
    builder_source_sha256 TEXT CHECK (builder_source_sha256 IS NULL OR (length(builder_source_sha256) = 64 AND builder_source_sha256 NOT GLOB '*[^0-9a-f]*')),
    materialized_split_sha256 TEXT CHECK (materialized_split_sha256 IS NULL OR (length(materialized_split_sha256) = 64 AND materialized_split_sha256 NOT GLOB '*[^0-9a-f]*')),
    runtime_sha256 TEXT CHECK (runtime_sha256 IS NULL OR (length(runtime_sha256) = 64 AND runtime_sha256 NOT GLOB '*[^0-9a-f]*')),
    candidate_superglm_sha256 TEXT CHECK (candidate_superglm_sha256 IS NULL OR (length(candidate_superglm_sha256) = 64 AND candidate_superglm_sha256 NOT GLOB '*[^0-9a-f]*')),
    validation_curve_reason TEXT,
    validation_curve_status TEXT CHECK ((validation_curve_status IS NULL AND validation_curve_reason IS NULL) OR (validation_curve_status IS NOT NULL AND validation_curve_status = 'COMPLETE' AND validation_curve_reason IS NULL) OR (validation_curve_status IS NOT NULL AND validation_curve_status = 'UNAVAILABLE' AND validation_curve_reason IS NOT NULL AND length(trim(validation_curve_reason)) > 0)),
    validation_source_model_run_id TEXT REFERENCES MODEL_RUN(model_run_id),
    effective_from TEXT,
...
CREATE TABLE IF NOT EXISTS pricing.PRICING_RATE_PACKAGE (
    ...
    build_fingerprint_sha256 TEXT CHECK (build_fingerprint_sha256 IS NULL OR (parent_rate_package_id IS NULL AND length(build_fingerprint_sha256) = 64 AND build_fingerprint_sha256 NOT GLOB '*[^0-9a-f]*')),
